...
export default function MachinesPage() {
  ...
  // command is generated by the effect below — instantly on open, then debounced
  // as SSH config changes — so it stays in sync with no manual "regenerate".
  ...
  // and the SSH config changes. Instant on open; debounced (400ms) on edits so
  // typing a port doesn't spray one-time tokens. Each regenerate supersedes the
  // prior token (which simply expires unused).
  useEffect(() => {
  ...
  }, [bootstrapModal.isOpen, sshEnabledInput, publicSSHInput, sshKeyInput, tunnelPortInput])
